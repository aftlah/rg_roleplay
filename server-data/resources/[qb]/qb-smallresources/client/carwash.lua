...
CreateThread(function()
    for k, v in pairs(Config.CarWash.locations) do
        if Config.UseTarget then
            local target = GetResourceState('ox_target') == 'started' and exports.qtarget
                or GetResourceState('qb-target') == 'started' and exports['qb-target']
            if not target then return end

            target:AddBoxZone('carwash_' .. k, v.coords, v.length, v.width, {
                name = 'carwash_' .. k,
                debugPoly = false,
                heading = v.heading,
                minZ = v.coords.z - 5,
