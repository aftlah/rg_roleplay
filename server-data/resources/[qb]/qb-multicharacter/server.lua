...
local function GiveStarterItems(source)
    ...
    for _, v in pairs(sharedStarterItems) do
        ...
        exports.ox_inventory:AddItem(src, v.item, v.amount, info)
    end
end
...
local function loadHouseData(src)
    ...
    local HouseGarages = {}
    local Houses = {}
    local ok, result = pcall(MySQL.query.await, 'SELECT * FROM houselocations', {})
    if not ok or not result or result[1] == nil then
        return
    end
